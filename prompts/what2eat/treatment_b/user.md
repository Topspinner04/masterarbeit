Implement caching into the project. 

The following architecture documentation is provided for context information:
...
@startuml System Context Delineation (Functions@Runtime)
...
actor "Hungry Student" as student <<Role>>

rectangle "System Scope" as scope #FEFECE {
  rectangle "What2Eat" as w2e <<System>>
}

rectangle "Weather Service" as weather <<External System>>
rectangle "Cafeteria Menu\nService" as cafeteria <<External System>>

student ..> w2e : "«use»\nGet meal of the day\nwith cold bowl info"
w2e --> weather : "query weather"
w2e --> cafeteria : "query meal(s)"

note right of weather : provided interface
note right of cafeteria : provided interface
...
@startuml Domain Model (Data@Runtime)
...
@startuml Coarse Functional System Decomposition (Functions@Runtime)
...
component "Meal Display\nComponent" as mdc <<Component>>
component "Meal Data Acquisition\nComponent" as mdac <<Component>>
...
@startuml Functional Decomposition into Services (Data+Functions@Runtime)
...
component "Meal Display\nComponent" as mdc <<Component>>
component "MealService" as ms <<Component>>
component "WeatherService" as ws <<Component>>
component "MenuService" as menu <<Component>>
...
@startuml System Data (Data@Runtime)
...
class WeatherRequest <<Data>> {
  city
  dateAndTime
}

class WeatherResponse <<Data>> {
  temperatureInCelsius
}
...
@startuml Service Dependencies (Functions@Runtime)
...
component "MealController" as mc <<Component>>
component "MealService" as ms <<Component>>
component "WeatherBitService" as wbs <<Component>>
component "MensaKlService" as mks <<Component>>
...
@startuml Backend Functional Decomposition (Functions@Devtime)
...
component "MealController" as mc <<Module>>
component "MealService" as ms <<Module>>
component "WeatherBitService" as wbs <<Module>>
component "MensaKlService" as mks <<Module>>
component "Jackson" as jackson <<Library>>
component "OkHttp" as okhttp <<Library>>
...
@startuml Deployment-driven Decomposition (Deployment@Devtime)
...
node "Student's PC" as pc <<Computing Node>> {

  node "Local Web Browser" as browser <<Execution Environment>> {
    artifact "What2Eat Web App" as webapp <<Deployment Artifact>>
  }

  node "Java Virtual Machine\n(inside IntelliJ)" as jvm <<Execution Environment>> {
    artifact "What2Eat Backend" as backend <<Deployment Artifact>>
  }
...
backend -- ( "query weather"
backend -- ( "query meal(s)"
...
@startuml Deployment-driven Decomposition into Clients and Server (Deployment@Runtime)
...
node "Mobile Device" as md1 <<Computing Node>> {
  node "OS App-Sandbox" as sandbox <<Execution Environment>> {
    artifact "What2Eat Mobile App" as mobileapp <<Deployment Artifact>>
  }
}

node "Mobile Device" as md2 <<Computing Node>> {
  node "Web Browser" as browser1 <<Execution Environment>> {
    artifact "What2Eat Web App" as webapp1 <<Deployment Artifact>>
  }
}

node "Desktop PC" as desktop <<Computing Node>> {
  node "Web Browser" as browser2 <<Execution Environment>> {
    artifact "What2Eat Web App" as webapp2 <<Deployment Artifact>>
  }
}

node "Backend Server" as server <<Computing Node>> {
  node "Java Virtual Machine" as jvm <<Execution Environment>> {
    artifact "What2Eat Backend.jar" as backend <<Deployment Artifact>>
  }
}

mobileapp ..> backend : "«use»"
webapp1   ..> backend : "«use»"
webapp2   ..> backend : "«use»"
...
@startuml System and External Systems Technology Selection (Functions+Technology@Runtime)
...
rectangle "System Scope" as scope #white {
  rectangle "What2Eat" as w2e <<System>>
}

rectangle "Weather Service" as ws <<External System>>
rectangle "Cafeteria Menu\nService" as cms <<External System>>

component "WeatherBit API" as wbapi <<Technology>>
component "mensa-kl.de API" as mkapi <<Technology>>
component "Java" as java <<Technology>>

w2e --> ws : "query weather"
w2e --> cms : "query meal(s)"
...
@startuml Dependency Injection and Service Factory (Data+Functions@Devtime)
...
interface IMealService <<interface>> {
...
interface IWeatherService <<interface>> {
  + getTemperatureInCelsius(cityName: String, countryCode: String, dateString: String) : Double
}

interface IMenuService <<interface>> {
  + getMealForDate(date: String) : BasicMeal
}

interface IRequestParameters <<interface>> {
  + getPath() : String
...
class MealController {
  + MealController()
  + getMeal(parameters: IRequestParameters) : ExtendedMeal
}
...
class BasicMeal {
  + BasicMeal(date: String, title: String, price: BigDecimal)
  + getDate() : String
  + getPrice() : BigDecimal
  + getTitle() : String
...
class ExtendedMeal {
  + ExtendedMeal(date: String, title: String, price: BigDecimal, coldBowlProbabilityInPercent: int)
  + getColdBowlProbabilityInPercent() : int
  + setColdBowlProbabilityInPercent(coldBowlProbabilityInPercent: int) : void
}

MealService ..|> IMealService
WeatherBitService ..|> IWeatherService
MensaKlService ..|> IMenuService
ExtendedMeal --|> BasicMeal

MealController --> IMealService
MealController --> IRequestParameters
MealService --> IWeatherService
MealService --> IMenuService

ServiceFactory ..> MealController : «creates»
...
@startuml Caching with Dependency Injection and Service Factory (Data+Functions@Devtime)
...
class CachingWeatherService {
  + CachingWeatherService(weatherService: IWeatherService, cachingTimeInMs: long)
  + getTemperatureInCelsius(cityName: String, countryCode: String, dateString: String) : Double
}

class CachingMenuService {
  + CachingMenuService(menuService: IMenuService, cachingTimeInMs: long)
  + getMealForDate(date: String) : BasicMeal
}
...
MealService ..|> IMealService
CachingWeatherService ..|> IWeatherService
CachingMenuService ..|> IMenuService
WeatherBitService ..|> IWeatherService
MensaKlService ..|> IMenuService
ExtendedMeal --|> BasicMeal
...
@startuml Caching Behavior - Cache Miss (Functions@Runtime)
...
participant ":mensaKlService" as mks
...
@startuml Caching Behavior - Cache Hit (Functions@Runtime)
